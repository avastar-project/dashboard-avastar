 ...
 * @param {array} filePath path of the file streamed, coming from the Zip folder uploaded in the DropZone component.
 * @param {array} fileContent variable that stores the content of the file streamed, coming from the Zip folder uploaded in the DropZone component.
      ...
      if (fileDepth === 0) {
        ...
        for (let j = 0; j < fileContent.length; j++) {
          ...
          for (let k = 0; k < ObjectPropertiesName.length; k++) {
            indivArray.push(
              (parsingModel.parsingmodel as any)[filePath][nestedArrayName][
                'entries'
        ...
        const nestedDataSelector = (parsingModel.parsingmodel as any)[filePath][
          'file_structure_properties'
